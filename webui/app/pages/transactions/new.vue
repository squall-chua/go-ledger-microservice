<template>
  <div>
    <div class="flex justify-between items-center mb-6">
      <div>
        <h1 class="text-3xl font-bold tracking-tight text-gray-900 dark:text-white">Record Transaction</h1>
        <p class="text-gray-500 mt-1">Double-entry ledger format. Debits and credits must zero out.</p>
      </div>
      <div class="flex gap-3">
        <UButton to="/transactions" color="neutral" variant="soft" icon="i-lucide-arrow-left">Back to Register</UButton>
      </div>
    </div>
    ...
    <UCard class="shadow-sm border-gray-200 dark:border-gray-800">
      <form @submit.prevent="submitTransaction" class="space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
          <UFormField label="Description / Note" required class="md:col-span-1">
            <UInput v-model="form.note" placeholder="e.g. Purchase Groceries" class="w-full" required />
          </UFormField>
          <UFormField label="Transaction Date" class="md:col-span-1">
            <UInput v-model="form.date" type="datetime-local" class="w-full" />
          </UFormField>
          <UFormField label="Currency" class="md:col-span-1">
            <USelectMenu v-model="form.currency" :items="CURRENCIES" class="w-full" />
          </UFormField>
        </div>
        ...
        <div class="space-y-4">
          <div v-for="(posting, index) in form.postings" :key="index" class="p-4 bg-gray-50 dark:bg-gray-900/50 rounded-lg border border-gray-200 dark:border-gray-800 relative group">
            <UButton 
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-8">
              <UFormField label="Account Type" class="md:col-span-1">
                <USelect v-model="posting.type" :items="ACCOUNT_TYPES" class="w-full" />
              </UFormField>
              <UFormField label="Account User" class="md:col-span-1">
                <UInput v-model="posting.user" placeholder="*" class="w-full" required />
              </UFormField>
              <UFormField label="Account Name" class="md:col-span-1">
                <UInput v-model="posting.name" placeholder="e.g. Checking" class="w-full" required />
              </UFormField>
              <UFormField label="Amount" class="md:col-span-1">
                <UInput v-model.number="posting.amount" type="number" step="0.01" placeholder="0.00" class="w-full" required />
              </UFormField>
            </div>
          </div>
        </div>
        ...
        <div class="flex justify-between items-center bg-gray-50 dark:bg-gray-900/50 p-4 rounded-lg border border-gray-200 dark:border-gray-800">
          <UButton color="neutral" variant="soft" icon="i-lucide-plus" @click="addPosting">Add Posting</UButton>
