<template>
  <div>
    <div class="flex items-center justify-between mb-6">
      <h1 class="text-3xl font-bold tracking-tight text-gray-900 dark:text-white">Overview</h1>
      <USelectMenu
    ...
    <div v-if="loading" class="grid grid-cols-1 md:grid-cols-3 gap-6">
      <USkeleton class="h-32 rounded-xl" v-for="i in 3" :key="i" />
    </div>

    <div v-else-if="error" class="bg-red-50 dark:bg-red-900/20 p-6 rounded-xl border border-red-200 dark:border-red-800 text-center">
      <UIcon name="i-lucide-triangle-alert" class="text-4xl text-red-500 mb-2" />
      <h3 class="text-lg font-medium text-red-800 dark:text-red-400">Failed to load overview</h3>
      <p class="text-red-600 dark:text-red-300 mt-1">{{ error.message || 'Please check your connection and authentication.' }}</p>
      <UButton color="error" variant="soft" class="mt-4" @click="fetchData">Retry</UButton>
    </div>
    ...
    <div v-else class="grid grid-cols-1 md:grid-cols-3 gap-6">
      ...
      >
        <div class="flex flex-col">
          ...
          <div class="mt-2 text-3xl font-bold">{{ formatCurrency(totals.assets, selectedCurrency) }}</div>
      ...
      >
        <div class="flex flex-col">
          ...
          <div class="mt-2 text-3xl font-bold">{{ formatCurrency(totals.revenue === 0 ? 0 : totals.revenue * -1, selectedCurrency) }}</div>
          <div class="mt-4 flex items-center text-sm text-emerald-100">
            <UIcon name="i-lucide-banknote" class="mr-1" />
            Income Streams
          </div>
        </div>
      </UCard>
      ...
      >
        <div class="flex flex-col">
          ...
          <div class="mt-2 text-3xl font-bold">{{ formatCurrency(totals.expenses, selectedCurrency) }}</div>
          <div class="mt-4 flex items-center text-sm text-rose-100">
            <UIcon name="i-lucide-trending-down" class="mr-1" />
            Outgoing Capital
          </div>
        </div>
      </UCard>
      ...
      <UCard class="rounded-xl shadow-sm border-gray-200 dark:border-gray-800">
        <template #header>
          <div class="flex items-center justify-center">
            <h3 class="text-lg font-semibold dark:text-gray-100 text-center w-full">Quick Actions</h3>
          </div>
        </template>
        <div class="flex gap-4 justify-center">
          <UButton to="/balances" color="neutral" variant="soft" icon="i-lucide-pie-chart" class="justify-center">View Balances</UButton>
          <UButton to="/transactions" color="neutral" variant="soft" icon="i-lucide-list" class="justify-center">View Register</UButton>
          <UButton to="/postings" color="neutral" variant="soft" icon="i-lucide-users" class="justify-center">View Postings</UButton>
          <UButton to="/transactions/new" color="primary" icon="i-lucide-plus" class="justify-center">Add Transaction</UButton>
        </div>
      </UCard>
    </div>
    ...
    <div class="mt-8">
      <UCard class="rounded-xl shadow-sm border-gray-200 dark:border-gray-800" :ui="{ body: 'p-0 sm:p-0', header: 'py-3' }">
        <template #header>
        ...
        <div class="overflow-x-auto max-h-64">
          <table class="w-full text-left border-collapse">
            <tbody class="divide-y divide-gray-200 dark:divide-gray-800">
              <tr v-if="loading">
                <td colspan="2" class="py-2 px-4 text-center text-xs text-gray-500">Loading balances...</td>
              </tr>
              <tr v-else-if="!rawBalances || rawBalances.length === 0">
                <td colspan="2" class="py-2 px-4 text-center text-xs text-gray-500">No balances found for {{ selectedCurrency }}</td>
              </tr>
              <tr v-else v-for="row in rawBalances" :key="row.account?.name + row.account?.type" class="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
