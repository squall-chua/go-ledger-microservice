...
func draftFromRequest(req *pb.RecordTransactionRequest) (repository.TransactionDraft, error) {
	...
	// The date is carried through untouched. Truncating it, capping how far
	// ahead it may sit and refusing a backdated one are one rule, and it is
	// settled in the repository under the row locks, where `latest` is true.
	sum := decimal.Zero
	currencyCode := ""
